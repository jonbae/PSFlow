...
const registry =
  fixtures.map(({ file }, i) => `import f${i} from ${JSON.stringify(file)};`).join("\n") +
  `\nexport default {\n` +
  fixtures.map(({ route }, i) => `  ${JSON.stringify(route)}: f${i},`).join("\n") +
  `\n};\n`;

const registryPlugin = {
  name: "psflow-fixtures",
  setup(b) {
    b.onResolve({ filter: /^psflow:fixtures$/ }, (args) => ({
      path: args.path,
      namespace: "psflow-fixtures",
    }));
    b.onLoad({ filter: /.*/, namespace: "psflow-fixtures" }, () => ({
      contents: registry,
      loader: "ts",
      resolveDir: repoRoot,
    }));
...
const result = await build({
  ...
  banner: {
    js:
      ...
      `// Side: ${side}. Fixtures: ${fixtures.length}, from the vendored @xyflow/react ${baseline}.\n` +
...
console.log(
  `driver [${side}]: ${fixtures.length} fixtures, library from \`${wants}\` → dist/${side}.js`
);
