...
Every row is covered by surface parity (`npm run parity:surface`) for **name
presence**. The `Gates today` columns list only what reaches further than that,
and a trailing `~` marks a gate that mounts or traverses the export without
asserting anything specific to it — it would not fail if the behaviour diverged.
...
| `ReactFlow` | component | `dual-run-dom` | conformance (generic-edges) + conformance (generic-props) + smoke | conformance (generic-nodes) + conformance (generic-pane) | The most-covered export — nodes, edges, pane, props specs all drive it. |
