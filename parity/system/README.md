...
The first run found nine classes of divergence across five fixtures — a missing
`aria-describedby`, edge coordinates carrying 2e-5 of accumulated error, node
DOM order sorted rather than in insertion order, and six more. They are on the
divergence backlog ([#22](https://github.com/jonbae/PSFlow/issues/22)),
**recorded and not fixed**: the fixing phase could not be specified before the
divergence set existed.
