...
Derived from 83 scenario(s) whose traces are on disk, captured against vendored `@xyflow/react` 12.11.0.
...
| `AriaLabelConfig` | dom | driven | `.react-flow__node[aria-describedby]` | `arrow-key-selected-node` +80 more |
| `Background` | dom | driven | `.react-flow__background` | `background-custom-bgcolor` +3 more |
| `BackgroundProps` | dom | driven | `.react-flow__background-pattern` | `background-custom-bgcolor` +3 more |
| `BaseEdge` | dom | driven | `path.react-flow__edge-path` | `arrow-key-selected-node` +79 more |
| `BaseEdgeProps` | dom | driven | `path.react-flow__edge-path[d]` | `arrow-key-selected-node` +79 more |
| `BezierEdge` | dom | driven | `.react-flow__edge-default` | `arrow-key-selected-node` +79 more |
| `BezierEdgeProps` | dom | driven | `.react-flow__edge-default path.react-flow__edge-path` | `arrow-key-selected-node` +79 more |
| `ColorMode` | dom | driven | `.react-flow.light` | `arrow-key-selected-node` +81 more |
| `ColorModeClass` | dom | driven | `.react-flow.dark` | `select-dark-color-mode` |
| `ControlButton` | dom | driven | `button.react-flow__controls-button` | `controls-horizontal` +3 more |
| `ControlButtonProps` | dom | driven | `button.react-flow__controls-button[title]` | `controls-horizontal` +3 more |
...
| `Controls` | dom | driven | `.react-flow__controls` | `controls-horizontal` +3 more |
| `DefaultEdgeOptions` | dom | hole | `.react-flow__edge-smoothstep` | declared — no issue owns it |
| `Edge` | dom | driven | `.react-flow__edge[data-id]` | `arrow-key-selected-node` +79 more |
...
| `EdgeWrapperProps` | dom | driven | `.react-flow__edge` | `arrow-key-selected-node` +79 more |
| `Handle` | dom | driven | `.react-flow__handle` | `arrow-key-selected-node` +80 more |
| `HandleProps` | dom | driven | `.react-flow__handle[data-handlepos]` | `arrow-key-selected-node` +80 more |
| `HandleType` | dom | driven | `.react-flow__handle.source` | `arrow-key-selected-node` +80 more |
| `KeyCode` | dom | driven | `.react-flow__nodesselection` | `selection-box-touch` +1 more |
| `MiniMap` | dom | driven | `.react-flow__minimap` | `minimap-all-nodes-hidden` +5 more |
| `MiniMapNode` | dom | driven | `.react-flow__minimap-node` | `minimap-custom-mask-colors` +3 more |
| `MiniMapProps` | dom | driven | `.react-flow__minimap-mask` | `minimap-all-nodes-hidden` +5 more |
| `Node` | dom | driven | `.react-flow__node[data-id]` | `arrow-key-selected-node` +80 more |
| `NodeOrigin` | dom | driven | `.react-flow__node[style*=translate]` | `arrow-key-selected-node` +80 more |
...
| `NodeWrapperProps` | dom | driven | `.react-flow__node` | `arrow-key-selected-node` +80 more |
| `Panel` | dom | driven | `.react-flow__panel` | `arrow-key-selected-node` +82 more |
| `PanelPosition` | dom | driven | `.react-flow__panel.top` | `mount-baseline--chrome-panels` +4 more |
| `PanelProps` | dom | driven | `.react-flow__panel` | `arrow-key-selected-node` +82 more |
| `ProOptions` | dom | driven | `.react-flow__attribution` | `arrow-key-selected-node` +82 more |
| `ReactFlow` | dom | driven | `.react-flow` | `arrow-key-selected-node` +82 more |
| `ReactFlowProps` | dom | driven | `.react-flow[data-testid]` | `arrow-key-selected-node` +82 more |
| `ReactFlowProvider` | dom | driven | `.react-flow` | `arrow-key-selected-node` +82 more |
...
| `Viewport` | dom | driven | `.react-flow__viewport[style*=transform]` | `arrow-key-selected-node` +82 more |
| `ViewportPortal` | dom | hole | `.react-flow__viewport-portal *` | declared — [issue](https://github.com/jonbae/PSFlow/issues/62) |
| `XYZPosition` | dom | driven | `.react-flow__node[style*=z-index]` | `arrow-key-selected-node` +80 more |
| `ZIndexMode` | dom | driven | `.react-flow__edges svg[style*=z-index]` | `arrow-key-selected-node` +79 more |
| `Connection` | callbacks | driven | `onConnect` | `connect-handle-to-handle` +3 more |
| `EdgeAddChange` | callbacks | hole | `onEdgesChange:add` | declared — no issue owns it |
| `EdgeChange` | callbacks | driven | `onEdgesChange` | `arrow-key-selected-node` +34 more |
| `EdgeMouseHandler` | callbacks | driven | `onEdgeClick, onEdgeDoubleClick, onEdgeContextMenu, onEdgeMouseEnter, onEdgeMouseMove, onEdgeMouseLeave` | `click-inside-interaction-width` +15 more |
...
| `FinalConnectionState` | callbacks | driven | `onConnectEnd` | `connect-handle-to-handle` +6 more |
| `IsValidConnection` | callbacks | driven | `isValidConnection` | `connect-handle-to-handle` +3 more |
| `NodeAddChange` | callbacks | driven | `onNodesChange:add` | `uncontrolled-update-node` |
| `NodeChange` | callbacks | driven | `onNodesChange` | `arrow-key-selected-node` +78 more |
| `NodeDimensionChange` | callbacks | driven | `onNodesChange:dimensions` | `arrow-key-selected-node` +78 more |
| `NodeMouseHandler` | callbacks | driven | `onNodeClick, onNodeDoubleClick, onNodeContextMenu, onNodeMouseEnter, onNodeMouseMove, onNodeMouseLeave` | `arrow-key-selected-node` +28 more |
| `NodePositionChange` | callbacks | driven | `onNodesChange:position` | `arrow-key-selected-node` +10 more |
| `NodeRemoveChange` | callbacks | driven | `onNodesChange:remove` | `delete-key-removes-node-and-edges` +1 more |
| `NodeReplaceChange` | callbacks | driven | `onNodesChange:replace` | `controls-horizontal` +2 more |
| `NodeSelectionChange` | callbacks | driven | `onNodesChange:select` | `arrow-key-selected-node` +20 more |
| `OnBeforeDelete` | callbacks | driven | `onBeforeDelete` | `delete-key-removes-edge` +4 more |
| `OnConnect` | callbacks | driven | `onConnect` | `connect-handle-to-handle` +3 more |
| `OnConnectEnd` | callbacks | driven | `onConnectEnd` | `connect-handle-to-handle` +6 more |
| `OnConnectStart` | callbacks | driven | `onConnectStart` | `connect-handle-to-handle` +8 more |
| `OnConnectStartParams` | callbacks | driven | `onConnectStart` | `connect-handle-to-handle` +8 more |
| `OnDelete` | callbacks | driven | `onDelete` | `delete-key-removes-edge` +2 more |
| `OnEdgesChange` | callbacks | driven | `onEdgesChange` | `arrow-key-selected-node` +34 more |
| `OnEdgesDelete` | callbacks | driven | `onEdgesDelete` | `delete-key-removes-edge` +2 more |
| `OnError` | callbacks | driven | `onError` | `click-reveals-default-toolbar` +27 more |
| `OnInit` | callbacks | hole | `onInit` | declared — no issue owns it |
| `OnMove` | callbacks | driven | `onMove` | `background-custom-bgcolor` +73 more |
| `OnMoveEnd` | callbacks | driven | `onMoveEnd` | `background-custom-bgcolor` +73 more |
| `OnMoveStart` | callbacks | driven | `onMoveStart` | `background-custom-bgcolor` +74 more |
| `OnNodeDrag` | callbacks | driven | `onNodeDragStart, onNodeDrag, onNodeDragStop` | `arrow-key-selected-node` +19 more |
| `OnNodesChange` | callbacks | driven | `onNodesChange` | `arrow-key-selected-node` +78 more |
...
| `OnSelectionChangeFunc` | callbacks | driven | `onSelectionChange` | `arrow-key-selected-node` +80 more |
| `OnSelectionChangeParams` | callbacks | driven | `onSelectionChange` | `arrow-key-selected-node` +80 more |
...
| `ReactFlowInstance` | api | driven | `getNode, getNodes, getEdge, getEdges, toObject, updateNode, updateEdge` | `arrow-key-selected-node` +49 more |
| `ReactFlowJsonObject` | api | driven | `toObject` | `arrow-key-selected-node` +49 more |
...
| `ViewportHelperFunctions` | api | driven | `zoomIn, zoomOut, zoomTo, getZoom, setViewport, getViewport, setCenter, fitBounds` | `arrow-key-selected-node` +49 more |
