...
| #4826 | Forward ref of the div inside Panel | [ReactFlow does not accept a ref](https://github.com/jonbae/PSFlow/issues/27) — one `forwardRef` deferral, already documented at `NodeWrapper.purs:28` and `EdgeWrapper.purs:25`. Two of that family of four have landed since this disposition was written: `<Panel />` — this PR's own subject — in boundary stage 4, and `<ReactFlow />` in #27. The two wrapper lines cited here are what is left, and the row stays where this table put it until they land. |
